...
export const generateQuestion = async (req, res) => {
    try {
        ...
        - Each question must be unique and cover a different topic.
        ...
        if (questionsArray.length < 5) {
            const sentenceSplit = aiResponse
                .replace(/\r/g, "")
                .split(/(?<=[.?!])\s+(?=[A-Z])/)
                .map(normalizeLine)
                .filter(q => q.length > 0);
            if (sentenceSplit.length >= 5) {
                questionsArray = sentenceSplit;
            }
        }
        ...
        const interview = await Interview.create({
            userId: user._id, role, experience, mode,
...
export const submitAnswer = async (req, res) => {
  ...
  `
        ...
        return res.status(500).json({message:`failed to submit ${error}`})
    }
}
